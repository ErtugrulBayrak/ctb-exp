...
def extract_json_from_text(text):
    ...
    if match:
        ...
        try:
            # Yaygın hataları temizlemeye çalışalım
            json_part = re.sub(r'//.*?$|/\*.*?\*/', '', json_part, flags=re.MULTILINE) # Yorumları sil
            json_part = json_part.replace('\n', '').replace('\r', '') # Satır sonlarını sil
            json_part = json_part.replace("'", '"') # Tek tırnakları çift tırnağa çevir (dikkatli kullanım!)
            # Sonda kalan virgülleri temizle (regex ile daha güvenli olabilir)
...
def get_buyuk_transferler(bitquery_api_key, min_usd_degeri=1000000, sure_dakika=60):
    """Bitquery kullanarak belirli bir değerin üzerindeki son transferleri çeker."""
    if not bitquery_api_key:
        print("UYARI (Bitquery): API anahtarı eksik, on-chain veri çekilemiyor.")
        return None
    # Anahtarın string olduğundan emin olalım (ekstra kontrol)
    if not isinstance(bitquery_api_key, str):
         print("HATA (Bitquery): API anahtarı .env dosyasından string olarak okunamadı.")
         return None

    print(f"🔗 Bitquery ile son {sure_dakika} dakikadaki >{min_usd_degeri:,}$ transferler sorgulanıyor...")

    # GraphQL Sorgusu (USDT ve USDC transferleri, Ethereum ağında)
    # Sorguyu daha okunabilir hale getirelim
    query = """
    ...
    # Zamanı hesapla (son X dakika)
    from datetime import datetime, timedelta, timezone
    try:
        start_time_dt = datetime.now(timezone.utc) - timedelta(minutes=sure_dakika)
        start_time_iso = start_time_dt.isoformat()
    except Exception as time_e:
         print(f"HATA (Bitquery Time Calc): Zaman hesaplaması başarısız. {time_e}")
         return None


    # API isteği için başlık ve değişkenler
    headers = {'Authorization': f'Bearer {bitquery_api_key}'} # DOĞRU FORMAT
    variables = {
        "limit": 30, # Biraz daha fazla çekelim, sonra USD'ye göre filtreleriz
        "offset": 0,
        "startTime": start_time_iso,
        # Token miktarına göre filtreleme (1 Milyon USDT/USDC)
        # GraphQL şeması Float beklediği için float yapalım
        "minAmountFloat": 1000000.0
    }

    response = None # Hata durumunda response değişkenine erişmek için başta tanımla
    try:
        response = requests.post('https://graphql.bitquery.io/',
                                 json={'query': query, 'variables': variables},
                                 headers=headers,
                                 timeout=40) # Timeout'u biraz daha artıralım
        response.raise_for_status() # HTTP 4xx veya 5xx hatası varsa exception fırlat
        data = response.json()

        # GraphQL seviyesinde hata kontrolü
        if 'errors' in data:
            # Hataları daha detaylı yazdır
        ...
        if not transfers:
            print("-> Belirtilen token miktarı kriterine uygun büyük transfer bulunamadı.")
            return None

        print(f"-> {len(transfers)} adet büyük transfer bulundu (token miktarına göre). USD değeri kontrol ediliyor...")

        # Sadece USD değeri belirlediğimiz eşiği geçenleri filtreleyelim
        ozet_listesi = []
        for t in transfers:
            amount_usd = t.get('amount_usd') # 0 default vermeyelim, None olabilir
            # amount_usd None olabilir veya sayısal olmayabilir, kontrol edelim
            if amount_usd is not None and isinstance(amount_usd, (int, float)) and amount_usd >= min_usd_degeri:
                 ...
                 ozet_listesi.append({
                     # Zaman formatını biraz kısaltalım
                     ...
                     "miktar_usd": f"${amount_usd:,.0f}", # Küsuratı atalım
                     "token": t.get('currency', {}).get('symbol', '?'),
                     # Tx hash'i şimdilik eklemeyelim, özeti kısa tutalım
                     # "tx_hash": t.get('transaction', {}).get('hash', '')
                 })

        if not ozet_listesi:
            print("-> USD değerine göre filtrelendiğinde büyük transfer kalmadı.")
            return None

        print(f"-> USD değeri >{min_usd_degeri:,}$ olan {len(ozet_listesi)} transfer özeti hazırlandı.")
        # Zamanına göre sıralayalım (en yeniden en eskiye) - zaten öyle geliyor ama garanti olsun
        ozet_listesi.sort(key=lambda x: x.get('zaman', ''), reverse=True)
        return ozet_listesi

    except requests.exceptions.HTTPError as http_err:
        # 401 hatasını daha spesifik loglayalım
        if response is not None and response.status_code == 401:
             print(f"HATA (Bitquery Auth): Kimlik doğrulama başarısız (401 Unauthorized). API Anahtarınızı (.env dosyasında) kontrol edin!")
             print(f"Gönderilen Header: {headers.get('Authorization', 'Header Yok')[:15]}...") # Anahtarın başını gösterelim
        elif response is not None:
             print(f"HATA (Bitquery HTTP): İstek başarısız oldu. Status: {response.status_code}, Yanıt: {response.text[:200]}")
        else:
            print(f"HATA (Bitquery HTTP): İstek başarısız oldu. {http_err}")
        return None
    except requests.exceptions.Timeout:
        print(f"HATA (Bitquery Request): API isteği zaman aşımına uğradı ({40}s).")
        return None
    except requests.exceptions.RequestException as e:
        print(f"HATA (Bitquery Request): API isteği başarısız oldu (Bağlantı sorunu olabilir). {e}"); return None
    except json.JSONDecodeError as e:
        print(f"HATA (Bitquery Response): API yanıtı JSON formatında değil. Hata: {e}")
        print(f"Alınan Yanıt (ilk 500kr): {response.text[:500] if response else 'Yanıt alınamadı'}")
        return None
    except Exception as e:
        import traceback
        print(f"HATA (Bitquery/Genel): Beklenmedik Hata -> {e}")
        # traceback.print_exc() # Geliştirme sırasında detaylı hata için
        return None
